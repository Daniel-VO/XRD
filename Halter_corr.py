"""
Created 01. February 2023 by Daniel Van Opdenbosch, Technical University of Munich
...
for i in list(filter(lambda a:'Halter' not in a,glob.glob(os.path.splitext(filepattern)[0]+'*.xy'))):
	...
	twotheta_deg,yobs,yh=np.delete(twotheta_deg,argscut),np.delete(yobs,argscut),np.delete(yh,argscut)

	yh=signal.savgol_filter(yh,101,1)
	yobs-=Cyh*yh
